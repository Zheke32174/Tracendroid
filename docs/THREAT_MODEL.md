...
**Status.** partial-closed — trust-anchor decisions resolved (`AUDIT_PLAN.md §§ 1.1, 1.2, 1.3`); the manifest format, trust pipeline (`PluginManifest`, `PluginSignatureVerifier`, `PluginPublisherTofuStore`, `PluginTrustChecker`), the TOFU prompt overlay, the trust settings screen, and the suspending install entry point (`TrustedPluginInstaller`) are all in place. The v1 policy decision: **the existing unsigned-plugin import path (`PackageManager.addPackageFileFromExternalStorage`) keeps its current behavior** — flipping it to refuse would break every plugin currently in the wild without a migration story. New plugin distribution channels (anything that wants the security posture of this row) adopt `TrustedPluginInstaller.verifyAndApprove()` and ship a signed `.toolpkg` with `manifest.json` + `manifest.sig`. The legacy path is grandfathered as an implementation reality; the trust pipeline is the path forward.

Cut-over criteria (when the legacy refusal flips on): a plugin ecosystem of meaningful size that ships signed bundles, plus a migration tool that lets users re-import legacy plugins as signed. Until both exist, refusing unsigned imports trades a working install flow for an aspirational rule — and the threat model values "working" over "aspirational" when the user already controls every grant via § 4.2.

**Location.** `app/src/main/java/com/ai/assistance/operit/core/tools/mcp/`, `core/tools/packTool/`, `core/tools/skill/` (legacy install paths — unchanged by v1). `core/plugintrust/{PluginManifest,PluginSignatureVerifier,PluginPublisherTofuStore,PluginTrustChecker,PluginInstallTofuRegistry,TrustedPluginInstaller}.kt` (trust pipeline for new channels). `ui/features/plugintrust/{PluginTrustScreen,PluginTofuPromptOverlay}.kt` (user surfaces). `docs/TOOLPKG_MANIFEST.md` (manifest format for plugin authors).
