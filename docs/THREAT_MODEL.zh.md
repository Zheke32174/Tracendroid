...
**状态。** design —— 信任根决策已解（`AUDIT_PLAN.md §§ 1.1、1.2、1.3`）；清单格式、信任管道（`PluginManifest`、`PluginSignatureVerifier`、`PluginPublisherTofuStore`、`PluginTrustChecker`）、TOFU 提示浮层、信任设置屏、可挂起安装入口（`TrustedPluginInstaller`）均已就位。仍未完成：将 `TrustedPluginInstaller.verifyAndApprove()` 接入既有的 `PackageManager.addPackageFileFromExternalStorage` 与 MCP / Skill 安装路径，以便在安装点拒绝未签名插件。信任管道今日可作为独立 API 调用；拒绝既有格式的安装是有意的切换，待那些安装路径更新时落地。

**位置。** `app/src/main/java/com/ai/assistance/operit/core/tools/mcp/`、`core/tools/packTool/`、`core/tools/skill/`（既有安装路径）。`core/plugintrust/{PluginManifest,PluginSignatureVerifier,PluginPublisherTofuStore,PluginTrustChecker,PluginInstallTofuRegistry,TrustedPluginInstaller}.kt`（信任管道）。`ui/features/plugintrust/{PluginTrustScreen,PluginTofuPromptOverlay}.kt`（用户接触面）。`docs/TOOLPKG_MANIFEST.md`（插件作者的清单格式）。
...
**状态。** design —— API 作用域已解（`AUDIT_PLAN.md § 1.6`）；调度器白名单 + Android 侧分类器布线随 shell 重建端到端落地。

**位置。** 见 `SHELL_REBUILD.md`。调度器路径：`app/src/main/assets/rootfs/operit-dispatcher.py`（白名单命令）。
...
- 中止动作通过 `HaltController.audit` 记录（有界环 StateFlow，64 条）。每条 `HaltEvent` 记录时间戳、by、verbatim 原因。保留 AI 完整推理快照是后续；v1 审计捕获最小化的 who/why/when 链。
- 中止是"主权而非终决"：`HaltController.clear()` 恢复活动。新的中止事件重新创造 Halted 状态；审计环无论状态如何都保留每条事件。

**状态。** partial-closed —— 中止控制 + 审计 + 跨所有当前爆破面的强制执行均已就位。按会话保留"中止当刻的 AI 推理快照"以及严格的"仅在执行器活动时"指示器变体作为后续跟踪。
...
- 跟踪后续：
    - 工作区绑定的 SSH 密钥与审计可能曝出的其他磁盘明文材料。

**状态。** closed —— vault 已就位；审计识别出的全部 API key、OAuth 令牌、bearer 凭据、PKCE verifier 均已加密驻留。若 SSH 密钥路径出现则为一个小后续；凭据存储行无其他未决项。
...
- 拒绝及用户的应对记录在 `DeclineRegistry.recent` 的审计环中。拒绝当刻的推理快照保留（与 § 4.7 的同一开放后续）作为同一个钩子跟踪。

**状态。** partial-closed —— 数据模型、注册器、审计环、UI 浮层均已就位。剩余缺口在后端侧布线：每个聊天后端（Anthropic、OpenAI、Google、Azure、本地 llama.cpp/MNN、proot 上的 codex/gemini-cli/claude-code）需要识别拒绝并调用 `DeclineRegistry.record(...)`。布线随 agent-core 接管每个后端逐个落地。
...
| 可审计 | 所有章节——任何特权动作 |
| 用户权威具有主权性（中止控制）| § 4.7（partial）|
| AI 是协作者（拒绝为一等结果）| § 4.13（partial）、§ 4.6 |
