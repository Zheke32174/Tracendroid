...
| `ShowerBinderReceiver` | `SHOWER_BINDER_READY` | 特权 binder 移交（Shower 内部服务器）| scheduled for removal（见 § 4.4）|
...
**规则。**
- 从 `app/build.gradle.kts` 与 `AndroidManifest.xml` 中移除 `libsu`、Shizuku、Shower 依赖。删除 `:showerclient` Gradle 模块。删除 `ShowerBinderReceiver`。移除 `ShizukuProvider` 清单声明；移除 `moe.shizuku.manager.permission.API_V23`。
- 将当前通过 `libsu` / Shizuku / Shower 调度的所有调用点迁移到 `app/src/main/java/com/ai/assistance/operit/core/tools/system/` 中的 AccessibilityService 路由等价物。无等价物的，该能力从项目中移除（见上述“接受的权衡”）。
- AccessibilityService 授权是按应用安装的（在系统设置中一次性授予、可在同一处撤销），但逐会话能力跟踪叠加在上面：会话默认不持有执行器能力；用户在首次使用时按会话授予（§ 4.7）。
- § 4.1 中的 Shower 接收器行在本 PR 中标为“scheduled for removal”；删除在后续实现 PR 中落地，该 PR 触及清单、构建脚本、Gradle 模块。

**状态。** open——清单清理、Gradle 依赖移除、调用点迁移跨多个实现 PR 落地。

**位置。** `app/build.gradle.kts`、`app/src/main/AndroidManifest.xml`、`app/src/main/java/com/ai/assistance/operit/core/tools/agent/ShowerBinderReceiver*.kt`、`:showerclient` Gradle 模块、遍布 `core/tools/system/` 的 libsu / Shizuku 调用点。
