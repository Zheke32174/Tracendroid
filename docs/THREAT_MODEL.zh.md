...
| 5 | chroot 内的官方订阅 CLI（codex、gemini-cli、claude-code）| 分发方 + chroot 内 Linux uid |
| 6 | 用户安装的插件（.toolpkg / MCP / Skill）| 插件发行方签名（待定——见 `AUDIT_PLAN.md`）|
| 6 | 远程 AI 提供方 | TLS / 提供方鉴权 |
| 7（最低）| chroot 内任意用户安装的二进制 | chroot 内 Linux uid |
...
| chroot 内订阅 CLI（codex / gemini-cli / claude-code）| 输入通道包括该 CLI 自配的提供方。输出校验：同样的门控 + chroot 隔离 |
...
| 订阅类 OAuth 令牌（codex / gemini-cli / claude-code）| chroot 文件系统 | 极高——长期有效、涉及金钱 |
...
| 应用 ↔ 特权 binder（Shizuku / Shower / libsu）| 三通道、调用点重复 | 单一抽象；逐会话授权；中止控制 |
| 应用 ↔ chroot 进程 | 当前破损（缺 rootfs）；隔离策略未定义 | chroot 是持久化边界；Android 侧的读取是只读、限定作用域、审计日志记录 |
| 应用 ↔ 远程 AI 提供方 | 仅 TLS；输出渲染未显式沙箱化 | 渲染路径不执行脚本；工具调用始终经过门控 |
| chroot CLI ↔ 订阅提供方 | 新接触面 | 各 CLI 自理 OAuth；令牌留在 chroot |
...
状态列取以下之一：**closed**（规则已落地）、**open**（规则尚未落地）、**design**（规则在设计中）、**broken**（子系统目前不可工作）。
...
| `ToolPkgDebugInstallReceiver` | `DEBUG_INSTALL_TOOLPKG` | 外部应用安装工具包 | open——亦应在 release 构建中移除的调试接触面 |
| `PackageDebugRefreshReceiver` | `DEBUG_REFRESH_PACKAGES` | 外部触发强制刷新插件 | open——调试用 |
| `ToolPkgComposeDslDebugDumpReceiver` | `DUMP_COMPOSE_DSL_UI` | 外部触发 UI 树转储 | open——调试用 |
...
| `ShowerBinderReceiver` | `SHOWER_BINDER_READY` | 特权 binder 移交 | open——需要与 Shower 服务端 APK 匹配的签名级权限 |
| `VoiceAssistantWidgetReceiver`、`ToolPkgDesktopWidgetReceiver` | `APPWIDGET_UPDATE` | 仅系统触发（可接受）| closed（仅系统）|

**规则。** 上表中状态为 "open" 的每一条获得一个签名级权限，挂接到我方发布密钥（调试/内部通道）或合法调用方的发行方（Workflow 接收器对应 Tasker；Shower binder 接收器对应 Shower 服务端 APK），并通过 build-type 专用清单从 release 构建中移除调试接收器。

**位置。** `app/src/main/AndroidManifest.xml`。Per-build-type 清单将位于 `app/src/release/AndroidManifest.xml` 与 `app/src/debug/AndroidManifest.xml`。
...
**规则。** 插件 JS 发起的工具调用穿过与 AI 发起的调用同样的逐次门控，调用上标注发起插件的身份。处于隔离区的插件不可注册处理器；升级是用户动作，审计日志记录。

**状态。** open。

**位置。** `app/src/main/java/com/ai/assistance/operit/core/tools/javascript/`、`core/tools/AIToolHandler.kt`、`core/tools/ToolRegistration.kt`。
...
### 4.4 特权 binder（libsu / Shizuku / Shower）

**发现。** 三条特权执行通道并存：
- `libsu 6.0.0`（topjohnwu）——通过 `su` 取得 root。
- Shizuku——通过 binder 服务在 ADB shell 级别执行（`rikka.shizuku` 依赖；清单中的 `ShizukuProvider`）。
- Shower——内部自研的 Shizuku 式服务端，配有自家客户端（`:showerclient` Gradle 模块）与 binder 移交（`ShowerBinderReceiver`）。

每一条都从多个调用点调用。三通道分支扩大了审计接触面，并导致逐会话授权的 UX 不一致。

**规则。**
- 单一的 Kotlin 抽象（`PrivilegedShell` 或类似名称——待定）隐藏使用的是哪条通道。调用点只使用该抽象。
- 抽象层强制默认拒绝：会话起始没有特权执行能力，直到用户授予。授权按会话生效，会话结束失效，可在会话进行中通过中止控制撤销（见 § Phone-as-actuator）。
- Shower binder 移交接收器由与 Shower 服务端 APK 匹配的签名级权限门控。

**状态。** design + open（抽象是新增；接收器门控是清单改动）。

**位置。** `app/src/main/java/com/ai/assistance/operit/core/tools/agent/ShowerBinderReceiver*.kt`、遍布 `core/tools/system/` 的 libsu/Shizuku 调用点、`:showerclient` 模块。

### 4.5 chroot 内订阅 OAuth

**发现。** 桥接至 CLI 的策略引入的新接触面。chroot 将托管首方 CLI（codex、gemini-cli、claude-code），每个 CLI 在自己的会话目录内持有长期有效的订阅令牌。

**规则。** 按 `SECURITY.md`：chroot 是持久化边界。Android 侧的访问是只读、按操作限定作用域、审计日志记录的。令牌轮换操作（在 CLI 自身之外发起时）不扩大作用域（CVE-2026-32922 教训）。

**状态。** design——依赖 shell 重建。

**位置。** 新增。位于 shell 重建范围（即将发布的 `docs/SHELL_REBUILD.md`）。
...
**发现。** 应用持有可对手机产生破坏性或可见副作用的权限：`SEND_SMS`、`READ_SMS`、`CALL_PHONE`、`REQUEST_INSTALL_PACKAGES`、`WRITE_SETTINGS`、`MANAGE_EXTERNAL_STORAGE`、`BIND_VOICE_INTERACTION`、`BIND_NOTIFICATION_LISTENER_SERVICE`、AccessibilityService、MediaProjection 前台服务。

**规则。**
- 任何 AI 会话起始时不默认拥有执行器能力。会话以零手机端能力开始；用户通过可见提示按会话授权。
- 当前会话存在执行器能力时，常驻指示器（状态栏 / 悬浮点）始终可见。隐藏指示器不是用户可配置的选项。
- 中止控制（按 `SECURITY.md` 原则 7）停止任何进行中的行动——AI 正在执行的调用、AI 启动的 chroot 进程、与会话绑定的前台服务——并撤销会话的执行器能力。中止控制可从常驻指示器一键触达。
...
**发现。** `app/build.gradle.kts` 第 74–75 行声明 `GITHUB_CLIENT_ID` 与 `GITHUB_CLIENT_SECRET` 作为 BuildConfig 字段，从 `local.properties` 读入。`_SECRET` 值如被填入，便进入 APK，被任何反编译者还原。

**规则。** 移动端 OAuth 采用 PKCE。本应用使用的 GitHub OAuth 流程（按 `docs/BUILDING.md`）目标深链 `operit://github-oauth-callback`，与仅使用 PKCE 的流程兼容。`_SECRET` 字段移除。

**状态。** open——小幅清单 + gradle + Kotlin 改动。

**位置。** `app/build.gradle.kts:74-75`、消费 `BuildConfig.GITHUB_CLIENT_SECRET` 的 OAuth 客户端代码（路径在审计时确定）。

### 4.9 凭据存储

**发现。** 当前许多凭据存于 DataStore preferences（提供方 API key、工作区绑定的 SSH 密钥）。DataStore 在磁盘上是明文的，除非由加密层包装。应用已依赖 `androidx.security:security-crypto`（基于 Tink 的 `EncryptedSharedPreferences`）。

**规则。** 每一项凭据——提供方 API key、SSH 密钥、工作区绑定、任何具有令牌性质的值——迁移至加密存储。迁移一次性、幂等，每条迁移记录产生一条审计日志条目。
...
**发现。** `AndroidManifest.xml` 中 `android:usesCleartextTraffic="true"`，由 `networkSecurityConfig="@xml/network_security_config"` 缓解。实际的明文白名单取决于 `res/xml/network_security_config.xml` 内容。

**规则。** 明文仅对明确的开发/内网来源（环回、RFC1918、命名的测试主机）开放。默认 HTTPS。配置文件的明文集合在本次 PR 与每次发布时审查。
...
**状态。** design——依赖 `AUDIT_PLAN.md § 1.7` 中定义的 `AgentOutcome` 数据模型。
...
|---|---|---|---|
| **ClawJacked**（Oasis Security）| 环回豁免限流 + 鉴权，再加"可信"来源自动配对 | `SECURITY.md` 红线：无环回豁免、无自动配对 | 未来设备端的本地 IPC 端点（HTTP / WebSocket / Unix socket），包括计划中的 Android↔chroot 桥 |
...
| `SECURITY.md` 默认 | 落地于 |
|---|---|
| 默认拒绝 | §§ 4.2、4.3、4.4、4.7 |
| 高影响动作逐次授权 | §§ 4.2、4.3、4.7 |
| 最小权限 | §§ 4.4、4.8 |
| 默认隔离 | §§ 4.2、4.5 |
| 构建产物中不嵌入秘密 | § 4.8 |
| 可审计 | 所有章节——任何特权动作 |
