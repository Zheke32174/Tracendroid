...
- 已迁移：
    - `GitHubAuthPreferences` —— `access_token`、`refresh_token`、`pending_oauth_state`、`pending_oauth_code_verifier`（四个含密字段）。非密元数据（`is_logged_in`、`user_info`、`last_login_time`、`auth_version`、`granted_scope`、`token_expires_at`、`token_type`）保留在 DataStore，让 UI 订阅的响应式 flow 继续工作。
    - `ExternalHttpApiPreferences` —— `bearer_token`。enabled / port 标记保留在 DataStore。
- 未迁移（跟踪后续，不阻塞本行的部分关闭）：
    - `ModelConfigManager` 的逐配置 blob 中含 `apiKey` 字段。blob 结构横跨众多调用点；干净的迁移需要结构性拆分（将 `apiKey` 从 blob 中移出）或逐 blob 一个 vault 条目；二者均范围在另起的提交中处理。
    - 工作区绑定的 SSH 密钥与审计可能曝出的其他磁盘明文材料。

**状态。** partial-closed —— vault 已就位；GitHub OAuth + external HTTP API 令牌（代码库中曝光最敏感的凭据）已迁移。ModelConfig API key + SSH 密钥为跟踪后续。

**位置。** `app/src/main/java/com/ai/assistance/operit/data/preferences/credentials/CredentialVault.kt`；迁移位于 `data/preferences/GitHubAuthPreferences.kt` 与 `data/preferences/ExternalHttpApiPreferences.kt`。
